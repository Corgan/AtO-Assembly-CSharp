...
public class SandboxManager : MonoBehaviour
{
  ...
  private void Start()
  {
    this.allButons = Functions.FindChildrenByName(this.sandboxOptions, new string[2]
    {
      "SandboxBox",
      "SandboxBoxCheck"
    });
  ...
  private void ShowEnableButtons()
  {
    if (this.isEnabled)
    {
      ...
      this.background.color = this.bgOn;
    }
    else
    {
      this.enabledBorder.gameObject.SetActive(false);
      this.background.color = this.bgOff;
    }
  ...
  {
    return this.sandBoxValues.ContainsKey(auxString) ? this.sandBoxValues[auxString] : 0;
  }
  ...
  public void SetObeliskSandbox(bool state)
  {
    if (state)
    {
      this.divinationCostDisable.gameObject.SetActive(true);
      this.maximumRerollsDisable.gameObject.SetActive(true);
      this.petsCostDisable.gameObject.SetActive(true);
    }
    else
    {
      this.divinationCostDisable.gameObject.SetActive(false);
      this.maximumRerollsDisable.gameObject.SetActive(false);
      this.petsCostDisable.gameObject.SetActive(false);
    }
  }
  ...
  public void SetObeliskSandboxMods()
  {
    foreach (KeyValuePair<string, List<int>> keyValuePair in this.keyValue)
    {
      if (keyValuePair.Key == "sbDivinationCost" || keyValuePair.Key == "sbPetsCost")
      {
        this.currentValue[keyValuePair.Key] = this.defaultValue[keyValuePair.Key];
        this.SetComboValue(keyValuePair.Key);
      }
  ...
  {
    return (GameManager.Instance.IsMultiplayer() && !NetworkManager.Instance.IsMaster() || PlayerManager.Instance.NgLevel != 0) && (bool) (UnityEngine.Object) HeroSelectionManager.Instance && !GameManager.Instance.IsLoadingGame() && (!GameManager.Instance.IsMultiplayer() || NetworkManager.Instance.IsMaster());
  }

  public static Transform[] FindChildren(Transform parent, string name)
  {
    return Array.FindAll<Transform>(parent.GetComponentsInChildren<Transform>(true), (Predicate<Transform>) (child => child.name == name));
  }
