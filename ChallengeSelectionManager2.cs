...
public class ChallengeSelectionManager2 : MonoBehaviour
{
	...
	private void Update()
	{
		if (GameManager.Instance != null && GameManager.Instance.GetDeveloperMode() && Input.GetKeyDown(KeyCode.F12))
		{
	...
	private void Start()
	{
		...
		assignButton0 = GameObject.Find("ChallengeSelect1").GetComponent<BotonGeneric>();
		assignButton1 = GameObject.Find("ChallengeSelect2").GetComponent<BotonGeneric>();
		assignButton2 = GameObject.Find("ChallengeSelect3").GetComponent<BotonGeneric>();
		...
		for (int j = 0; j < 4; j++)
		{
			sideCharacters.ShowChallengeButtons(j, state: false);
		}
	}
	...
	private void GeneratePacks()
	{
		...
		{
			Hero hero = theTeam[i];
			Enums.CardClass cardClass = (Enums.CardClass)Enum.Parse(typeof(Enums.CardClass), Enum.GetName(typeof(Enums.HeroClass), hero.HeroData.HeroClass));
			for (int j = 0; j < 3; j++)
			{
				List<PackData> list = GetPackListForClass(cardClass, hero.HeroData.HeroSubClass.Id, j).ShuffleList();
				for (int k = 0; k < 6; k++)
				{
					List<CardData> list2 = new List<CardData>();
					if (list[k].Card0 != null)
					{
						list2.Add(list[k].Card0);
					}
					if (list[k].Card1 != null)
					{
						list2.Add(list[k].Card1);
					}
					if (list[k].Card2 != null)
					{
						list2.Add(list[k].Card2);
					}
					if (list[k].Card3 != null)
					{
						list2.Add(list[k].Card3);
					}
					if (list[k].Card4 != null)
					{
						list2.Add(list[k].Card4);
					}
					List<CardData> list3 = new List<CardData>();
					if (list[k].CardSpecial0 != null)
					{
						list3.Add(list[k].CardSpecial0);
					}
					if (list[k].CardSpecial1 != null)
					{
						list3.Add(list[k].CardSpecial1);
					}
					list2 = list2.ShuffleList();
					list3 = list3.ShuffleList();
					string[] array = new string[cardsForPack];
					for (int l = 0; l < cardsForPack; l++)
					{
						array[l] = Functions.GetCardByRarity(UnityEngine.Random.Range(0, 100), list2[l], isChallenge: true);
					}
					...
					cardsDraftedPackname.Add(stringBuilder.ToString(), list[k].PackName);
					cardsDraftedPackid.Add(stringBuilder.ToString(), list[k].PackId);
					bool flag = false;
					string text = "";
					...
					while (!flag && num2 < 500)
					{
						text = Functions.GetCardByRarity(UnityEngine.Random.Range(0, 100), list3[num], isChallenge: true) + "_" + hero.HeroData.HeroSubClass.Id;
						while (cardsDraftedSpecial.ContainsValue(text))
						{
							num++;
							if (num >= list3.Count)
							{
						...
						{
							CardData cardData = Globals.Instance.GetCardData(array2[0], instantiate: false);
							if (cardData != null && cardData.CardUpgraded != Enums.CardUpgraded.No)
							{
								flag = true;
							}
						}
					...
					cardsDraftedSpecial.Add(stringBuilder.ToString(), text);
				}
			}
		}
	}
	...
	public void Reroll(int _heroId, bool fromMP = false)
	{
		...
		if (!heroRerollUsed[_heroId])
		{
			heroRerollUsed[_heroId] = true;
			if (_heroId == currentHeroIndex)
			{
				ShowCardList(_heroId);
			}
		}
	}
	...
	private void ShowCardList(int heroId)
	{
		...
		if (heroRoundArr[heroId] < 3)
		{
			...
			{
				string key = heroId + "_" + heroRoundArr[heroId] + "_" + i;
				CardCraftManager.Instance.AssignChallengeTitle(num3, Functions.UppercaseFirst(Texts.Instance.GetText(cardsDraftedPackname[key])));
				for (int j = 0; j < cardsForPack; j++)
				{
					CardCraftManager.Instance.AssignChallengeCard(heroId, num3, j, cardsDrafted[key][j]);
				}
				num3++;
			}
		...
		else if (heroRoundArr[heroId] == 3)
		{
			...
			for (int k = 0; k < 3; k++)
			{
				...
				stringBuilder.Append('_');
				string key2 = heroId + "_" + k + "_" + array[k];
				CardCraftManager.Instance.AssignChallengeTitle(k, Functions.UppercaseFirst(Texts.Instance.GetText(cardsDraftedPackname[key2])));
			}
		...
		CardCraftManager.Instance.ShowChallengeReroll(!heroRerollUsed[heroId]);
	...
	public List<PackData> GetPackListForClass(Enums.CardClass cardClass, string subclassId, int round)
	{
		...
		foreach (KeyValuePair<string, PackData> item in Globals.Instance.PackDataSource)
		{
			if (item.Value.PackClass == cardClass && item.Value.RequiredClass == null)
			{
				list.Add(item.Value);
			}
		}
	...
	public void SelectPack(int _heroId, int _pack, bool fromMP = false)
	{
		...
		if (heroRoundArr[_heroId] < 3)
		{
			...
			stringBuilder.Append("_");
			stringBuilder.Append(heroRoundArr[_heroId]);
			stringBuilder.Append("_");
	...
	private void FinishDraw()
	{
		...
		if (!GameManager.Instance.IsMultiplayer())
		{
			for (int j = 0; j < 4; j++)
			{
				if (perkDrafted != null && perkDrafted[j] != null && perkDrafted[j].Count < 4)
				{
					flag = false;
					break;
				}
		...
		else
		{
			for (int k = 0; k < 4; k++)
			{
				if (AtOManager.Instance.GetHero(k).Owner == NetworkManager.Instance.GetPlayerNick() && perkDrafted != null && perkDrafted[k] != null && perkDrafted[k].Count < 4)
				{
					flag = false;
					break;
				}
