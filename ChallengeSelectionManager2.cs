...
#nullable disable
public class ChallengeSelectionManager2 : MonoBehaviour
{
  private PhotonView photonView;
  public BotonGeneric assignButton0;
  public BotonGeneric assignButton1;
  public BotonGeneric assignButton2;
  ...
  private bool[] heroRerollUsed = new bool[4];
  ...
  public static ChallengeSelectionManager2 Instance { get; private set; }
  ...
  private void Awake()
  {
    ...
    {
      if ((UnityEngine.Object) ChallengeSelectionManager2.Instance == (UnityEngine.Object) null)
        ChallengeSelectionManager2.Instance = this;
      else if ((UnityEngine.Object) ChallengeSelectionManager2.Instance != (UnityEngine.Object) this)
  ...
  private void Update()
  {
    if (!((UnityEngine.Object) GameManager.Instance != (UnityEngine.Object) null) || !GameManager.Instance.GetDeveloperMode() || !Input.GetKeyDown(KeyCode.F12))
      return;
  ...
  private void Start()
  {
    ...
    {
      this.heroRoundArr[key] = 0;
      if (!this.perkDrafted.ContainsKey(key))
        this.perkDrafted.Add(key, new List<int>());
      else
        this.perkDrafted[key] = new List<int>();
    }
    this.theTeam = AtOManager.Instance.GetTeam();
    this.SetDefaultCards(0);
    this.SetDefaultCards(1);
    this.SetDefaultCards(2);
    this.SetDefaultCards(3);
    AtOManager.Instance.DoChallengeShop();
    this.assignButton0 = GameObject.Find("ChallengeSelect1").GetComponent<BotonGeneric>();
    this.assignButton1 = GameObject.Find("ChallengeSelect2").GetComponent<BotonGeneric>();
    this.assignButton2 = GameObject.Find("ChallengeSelect3").GetComponent<BotonGeneric>();
    this.perkChallengeItems = CardCraftManager.Instance.perkChallengeItems;
    ...
    this.StartCoroutine(this.NextHero(false));
    this.ShowCardList(this.currentHeroIndex);
    for (int hero = 0; hero < 4; ++hero)
      this.sideCharacters.ShowChallengeButtons(hero, false);
  }
  ...
  private void GeneratePacks()
  {
    ...
    for (int index1 = 0; index1 < 4; ++index1)
    {
      ...
      Enums.CardClass cardClass = (Enums.CardClass) Enum.Parse(typeof (Enums.CardClass), Enum.GetName(typeof (Enums.HeroClass), (object) hero.HeroData.HeroClass));
      for (int round = 0; round < 3; ++round)
      {
        List<PackData> packDataList = this.GetPackListForClass(cardClass, hero.HeroData.HeroSubClass.Id, round).ShuffleList<PackData>();
        for (int index2 = 0; index2 < 6; ++index2)
        {
          List<CardData> ts1 = new List<CardData>();
          if ((UnityEngine.Object) packDataList[index2].Card0 != (UnityEngine.Object) null)
            ts1.Add(packDataList[index2].Card0);
          if ((UnityEngine.Object) packDataList[index2].Card1 != (UnityEngine.Object) null)
            ts1.Add(packDataList[index2].Card1);
          if ((UnityEngine.Object) packDataList[index2].Card2 != (UnityEngine.Object) null)
            ts1.Add(packDataList[index2].Card2);
          if ((UnityEngine.Object) packDataList[index2].Card3 != (UnityEngine.Object) null)
            ts1.Add(packDataList[index2].Card3);
          if ((UnityEngine.Object) packDataList[index2].Card4 != (UnityEngine.Object) null)
            ts1.Add(packDataList[index2].Card4);
          List<CardData> ts2 = new List<CardData>();
          if ((UnityEngine.Object) packDataList[index2].CardSpecial0 != (UnityEngine.Object) null)
            ts2.Add(packDataList[index2].CardSpecial0);
          if ((UnityEngine.Object) packDataList[index2].CardSpecial1 != (UnityEngine.Object) null)
            ts2.Add(packDataList[index2].CardSpecial1);
          ...
          for (int index3 = 0; index3 < this.cardsForPack; ++index3)
            strArray1[index3] = Functions.GetCardByRarity(UnityEngine.Random.Range(0, 100), cardDataList1[index3], true);
          ...
          this.cardsDraftedPackname.Add(stringBuilder.ToString(), packDataList[index2].PackName);
          this.cardsDraftedPackid.Add(stringBuilder.ToString(), packDataList[index2].PackId);
          bool flag = false;
          string str = "";
          int index4 = UnityEngine.Random.Range(0, cardDataList2.Count);
          for (int index5 = 0; !flag && index5 < 500; ++index5)
          {
            for (str = Functions.GetCardByRarity(UnityEngine.Random.Range(0, 100), cardDataList2[index4], true) + "_" + hero.HeroData.HeroSubClass.Id; this.cardsDraftedSpecial.ContainsValue(str); str = Functions.GetCardByRarity(UnityEngine.Random.Range(0, 100), cardDataList2[index4], true) + "_" + hero.HeroData.HeroSubClass.Id)
            {
              ++index4;
              if (index4 >= cardDataList2.Count)
                index4 = 0;
            }
  ...
  public void Reroll(int _heroId, bool fromMP = false)
  {
    ...
    if (this.heroRerollUsed[_heroId])
      return;
    this.heroRerollUsed[_heroId] = true;
    if (_heroId != this.currentHeroIndex)
      return;
    this.ShowCardList(_heroId);
  }
  ...
  public void ChangeCharacter(int _heroIndex)
  {
    ...
    this.ShowCardList(_heroIndex);
  }

  private void ShowCardList(int heroId)
  {
    ...
    if (this.heroRoundArr[heroId] < 3)
    {
      ...
      {
        string key = heroId.ToString() + "_" + this.heroRoundArr[heroId].ToString() + "_" + index.ToString();
        ...
        for (int row = 0; row < this.cardsForPack; ++row)
          CardCraftManager.Instance.AssignChallengeCard(heroId, block, row, this.cardsDrafted[key][row]);
        ++block;
      }
    ...
    CardCraftManager.Instance.ShowChallengeReroll(!this.heroRerollUsed[heroId]);
    ...
    int round)
  {
    ...
    foreach (KeyValuePair<string, PackData> keyValuePair in Globals.Instance.PackDataSource)
    {
      if (keyValuePair.Value.PackClass == cardClass && (UnityEngine.Object) keyValuePair.Value.RequiredClass == (UnityEngine.Object) null)
        packListForClass.Add(keyValuePair.Value);
    }
  ...
  public void SelectPack(int _heroId, int _pack, bool fromMP = false)
  {
    ...
    if (this.heroRoundArr[_heroId] < 3)
    {
      ...
      stringBuilder.Append("_");
      stringBuilder.Append(this.heroRoundArr[_heroId]);
      stringBuilder.Append("_");
    ...
    this.heroRerollUsed[_heroId] = false;
  ...
  private void FinishDraw()
  {
    ...
    if (!GameManager.Instance.IsMultiplayer())
    {
      for (int key = 0; key < 4; ++key)
      {
        if (this.perkDrafted != null && this.perkDrafted[key] != null && this.perkDrafted[key].Count < 4)
        {
          flag = false;
          break;
        }
    ...
    else
    {
      for (int index = 0; index < 4; ++index)
      {
        if (AtOManager.Instance.GetHero(index).Owner == NetworkManager.Instance.GetPlayerNick() && this.perkDrafted != null && this.perkDrafted[index] != null && this.perkDrafted[index].Count < 4)
        {
          flag = false;
          break;
        }
