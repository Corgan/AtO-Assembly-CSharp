...
public class AlertManager : MonoBehaviour
{
  ...
  {
    if (this.popupT.gameObject.activeSelf)
    {
      this.alertText.text = "";
  ...
  private void ShowAlert(bool isInput)
  {
    ...
    this.popupT.gameObject.SetActive(true);
    this.playersT.gameObject.SetActive(false);
    ...
    {
      this.alertText.GetComponent<RectTransform>().sizeDelta = new Vector2(0.0f, 190f);
  ...
  {
    this.AlertConfirm(string.Format(Texts.Instance.GetText("youMustSelectHero"), (object) _charName));
  }

  public void HoverOn() => GameManager.Instance.PlayAudio(AudioManager.Instance.soundButtonHover);
  ...
  public void ShowPlayers()
  {
    ...
    this.SetInitialPosition();
    this.gameObject.SetActive(true);
    this.popupT.gameObject.SetActive(false);
    this.playersT.gameObject.SetActive(true);
    GameManager.Instance.PlayLibraryAudio("ui_menu_popup_01");
    if ((bool) (Object) PopupManager.Instance)
      PopupManager.Instance.ClosePopup();
    if ((bool) (Object) MapManager.Instance)
  ...
  public void SetPlayers()
  {
    ...
    {
      string playerNickPosition = NetworkManager.Instance.GetPlayerNickPosition(index);
      if (!playerNickPosition.IsNullOrEmpty())
      {
        this.playerList[index].gameObject.SetActive(true);
        this.playerList[index].SetPlayer(index, playerNickPosition);
      }
      else
        this.playerList[index].gameObject.SetActive(false);
    }
  }

  public void UpdatePlayer(int slot)
  {
    if (slot <= -1 || slot >= this.playerList.Count || !((Object) this.playerList[slot] != (Object) null))
      return;
    this.playerList[slot].SetDescription();
  }
