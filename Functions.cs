...
public static class Functions
{
  ...
  private static Dictionary<string, string> replacements = new Dictionary<string, string>()
  {
    ...
    {
      "<br><br>",
      "<br><line-height=50%><br></line-height>"
    },
    {
      "<br1>",
      "\n"
  ...
  public static string CheckIfSavegameIsCompatible(GameData gameData)
  {
    if (gameData == null)
      return "";
    int number1 = Functions.GameVersionToNumber(gameData.Version);
  ...
  {
    return string.IsNullOrEmpty(str) ? string.Empty : char.ToUpper(str[0]).ToString() + str.Substring(1).ToLower();
  }
  ...
  {
    return (double) num % 1.0 < 0.5 ? Mathf.FloorToInt(num) : Mathf.CeilToInt(num);
  }
  ...
  {
    return positions <= 0 ? Mathf.Floor(number) : Mathf.Floor(number * Mathf.Pow(10f, (float) positions)) / Mathf.Pow(10f, (float) positions);
  }
  ...
  public static string GetMadnessBonusText(int value)
  {
    ...
    switch (value)
    {
      ...
      case 9:
        stringBuilder.Append(Texts.Instance.GetText("madnessExhaust"));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessMonsterHp"), (object) " <color=#E5A44E>+45% / +60% / +65% / +70%</color> ", (object) Texts.Instance.GetText("madnessPerAct")));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessMonsterSpeed"), (object) " <color=#E5A44E>+2 / +3 / +3 / +4</color> ", (object) Texts.Instance.GetText("madnessPerAct")));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessMonsterDamageTaken"), (object) " <color=#E5A44E>-2 / -3 / -3 / -4</color> ", (object) Texts.Instance.GetText("madnessPerAct")));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessMonsterDamageDone"), (object) " <color=#E5A44E>+2 / +3 / +3 / +4</color> ", (object) Texts.Instance.GetText("madnessPerAct")));
        stringBuilder.Append("<br>");
        stringBuilder.Append(Texts.Instance.GetText("madnessDeadlyInjuries"));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessNoChest"), (object) "250"));
        stringBuilder.Append("<br>");
        stringBuilder.Append(Texts.Instance.GetText("madnessNoSupplyExchange"));
        stringBuilder.Append("<br>");
        stringBuilder.Append(string.Format(Texts.Instance.GetText("madnessHeroEnergy"), (object) " <color=#E5A44E>-2</color> "));
    ...
    string input = Functions.PregReplaceIcon(stringBuilder.ToString());
    if (Functions.SpaceBeforePercentSign())
      input = Regex.Replace(Regex.Replace(input, "(?<!\\s)%", " %"), " \\/ ", "<color=#AAA><size=-.3><voffset=.2> / </voffset></size></color>");
    return input;
  }
  ...
  public static string FormatString(string text)
  {
    ...
    foreach (KeyValuePair<string, string> replacement in Functions.replacements)
  ...
  {
    string input = Regex.Replace(new StringBuilder(text).Replace("<gl>", "<color=#5E3016>").Replace("</gl>", "</color>").Replace("<bl>", "<color=#263ABC>").Replace("</bl>", "</color>").Replace("<rd>", "<color=#720070>").Replace("</rd>", "</color>").Replace("<act>", "<size=-.15><color=#444>").Replace("</act>", "</size></color>").ToString(), "\\<i=(\\w+)\\>", "<size=+.1><voffset=-.1><space=.2> <sprite name=$1></voffset></size>");
    string pattern = "<crd=(.*?),(.*?)>";
  ...
  {
    return str != "" ? Regex.Replace(str, "[^\\u0000-\\u007F]+", string.Empty) : "";
  }
  ...
  public static string Sanitize(string str, bool doLowerCase = true)
  {
    if (str != "")
    {
      ...
      if (str1 != "")
        return doLowerCase ? str1.ToLower().Trim().Replace("'", "") : str1.Trim().Replace("'", "");
    }
    return "";
  }
  ...
  {
    CardData dataFromCardData = Functions.GetCardDataFromCardData(Globals.Instance.GetCardData(cardId.ToLower(), false), "");
    for (int index = 0; index < cardList.Count; ++index)
    {
      if (Functions.GetCardDataFromCardData(Globals.Instance.GetCardData(cardList[index], false), "").Id == dataFromCardData.Id)
        return true;
    }
    return false;
  }
  ...
  {
    return rarity < 15 && _cardData.CardRarity == Enums.CardRarity.Common || rarity >= 15 && rarity < 45 && _cardData.CardRarity == Enums.CardRarity.Uncommon || rarity >= 45 && rarity < 80 && _cardData.CardRarity == Enums.CardRarity.Rare || rarity >= 80 && rarity < 95 && _cardData.CardRarity == Enums.CardRarity.Epic || rarity >= 95 && rarity < 100 && _cardData.CardRarity == Enums.CardRarity.Mythic;
  }
  ...
  public static int GetListClosestIndexToMousePosition(List<Transform> theList, bool checkUiItems = false)
  {
    ...
    if (KeyboardManager.Instance.IsActive() | checkUiItems)
      b = Input.mousePosition + new Vector3(0.0f, 0.0f, 10f);
  ...
  public static string HideEmail(string email)
  {
    ...
    return startIndex <= 0 ? email : email.Substring(0, 1) + new string('*', startIndex - 1) + email.Substring(startIndex);
  }
  ...
  {
    return Globals.Instance.CurrentLang == "es" || Globals.Instance.CurrentLang == "fr" || Globals.Instance.CurrentLang == "sv";
  }

  public static bool CompatibleVersion(string version1, string version2)
  {
    string[] strArray1 = version1.Split('.', StringSplitOptions.None);
    string[] strArray2 = version2.Split('.', StringSplitOptions.None);
    for (int index = 0; index < 3; ++index)
    {
      if ((strArray1.Length <= index ? "0" : strArray1[index]) != (strArray2.Length <= index ? "0" : strArray2[index]))
        return false;
    }
    return true;
  }
