...
public class CharPopup : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    {
      GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(GameManager.Instance.CardPrefab, Vector3.zero, Quaternion.identity, this.groupSingularityCardsContainer);
      CardItem component = gameObject.GetComponent<CardItem>();
      gameObject.name = "card_" + index.ToString();
  ...
  private void Start()
  {
    ...
    if (GameManager.Instance.IsSingularity())
      this.buttonSingularityCards.gameObject.SetActive(true);
    else
      this.buttonSingularityCards.gameObject.SetActive(false);
    this.HideNow();
  }
  ...
  public void Init(SubClassData _scd, bool doStats = true)
  {
    ...
    this._Slashing.text = !Functions.SpaceBeforePercentSign() ? resistSlashing.ToString() + "%" : resistSlashing.ToString() + " %";
    if (resistSlashing > 0)
    ...
    this._Blunt.text = !Functions.SpaceBeforePercentSign() ? resistBlunt.ToString() + "%" : resistBlunt.ToString() + " %";
    if (resistBlunt > 0)
    ...
    this._Piercing.text = !Functions.SpaceBeforePercentSign() ? resistPiercing.ToString() + "%" : resistPiercing.ToString() + " %";
    if (resistPiercing > 0)
    ...
    this._Fire.text = !Functions.SpaceBeforePercentSign() ? resistFire.ToString() + "%" : resistFire.ToString() + " %";
    if (resistFire > 0)
    ...
    this._Cold.text = !Functions.SpaceBeforePercentSign() ? resistCold.ToString() + "%" : resistCold.ToString() + " %";
    if (resistCold > 0)
    ...
    this._Lightning.text = !Functions.SpaceBeforePercentSign() ? resistLightning.ToString() + "%" : resistLightning.ToString() + " %";
    ...
    this._Mind.text = !Functions.SpaceBeforePercentSign() ? resistMind.ToString() + "%" : resistMind.ToString() + " %";
    if (resistMind > 0)
    ...
    this._Holy.text = !Functions.SpaceBeforePercentSign() ? resistHoly.ToString() + "%" : resistHoly.ToString() + " %";
    if (resistHoly > 0)
    ...
    this._Shadow.text = !Functions.SpaceBeforePercentSign() ? resistShadow.ToString() + "%" : resistShadow.ToString() + " %";
    if (resistShadow > 0)
    ...
    if (!_scd.Sku.IsNullOrEmpty())
    {
      ...
      this.dlcCharacter.GetComponent<PopupText>().text = string.Format(Texts.Instance.GetText("requiredDLC").Replace("#FFF", "#CEA843"), (object) SteamManager.Instance.GetDLCName(_scd.Sku));
    }
    else
      this.dlcCharacter.gameObject.SetActive(false);
    if (!GameManager.Instance.IsSingularity())
    {
      this.initialCards.gameObject.SetActive(true);
      this.checkSingularityCards.gameObject.SetActive(false);
    }
    else
    {
      this.initialCards.gameObject.SetActive(false);
      this.checkSingularityCards.gameObject.SetActive(true);
    }
  }
  ...
  private void SetHeroAnimated(GameObject heroSkin = null)
  {
    ...
    if ((UnityEngine.Object) original != (UnityEngine.Object) null)
    {
      ...
      if (this.SCD.SubClassName.ToLower() == "alchemist")
        this.heroAnimated.transform.localPosition = new Vector3(0.2f, 0.1f, 0.0f);
      else if (this.SCD.SubClassName.ToLower() == "cleric")
        this.heroAnimated.transform.localPosition = new Vector3(0.0f, -0.17f, 0.0f);
      else if (this.SCD.SubClassName.ToLower() == "engineer")
      {
        this.heroAnimated.transform.localScale = new Vector3(0.95f, 0.95f, 1f);
        this.heroAnimated.transform.localPosition = new Vector3(-0.125f, -0.1f, 0.0f);
      }
      else if (this.SCD.SubClassName.ToLower() == "minstrel")
        this.heroAnimated.transform.localPosition = new Vector3(-0.1f, 0.0f, 0.0f);
      else if (this.SCD.SubClassName.ToLower() == "paladin")
        this.heroAnimated.transform.localPosition = new Vector3(0.0f, -0.17f, 0.0f);
      else if (this.SCD.SubClassName.ToLower() == "priest")
        this.heroAnimated.transform.localPosition = new Vector3(0.15f, -0.15f, 0.0f);
      else if (this.SCD.SubClassName.ToLower() == "prophet")
        this.heroAnimated.transform.localScale = new Vector3(1f, 1f, 1f);
      else if (this.SCD.SubClassName.ToLower() == "queen" || this.SCD.SubClassName.ToLower() == "valkyrie")
      {
        this.heroAnimated.transform.localScale = new Vector3(0.95f, 0.95f, 1f);
        this.heroAnimated.transform.localPosition = new Vector3(0.15f, -0.45f, 0.0f);
      }
      else if (this.SCD.SubClassName.ToLower() == "voodoowitch")
      {
        this.heroAnimated.transform.localScale = new Vector3(0.95f, 0.95f, 1f);
      ...
      else if (this.SCD.HeroClass == Enums.HeroClass.Warrior)
      {
        ...
        if (this.SCD.SubClassName.ToLower() == "bandit")
          this.heroAnimated.transform.localScale = new Vector3(0.93f, 0.93f, 1f);
        else if (this.SCD.SubClassName.ToLower() == "sentinel")
          this.heroAnimated.transform.localScale = new Vector3(0.96f, 0.96f, 1f);
        else if (this.SCD.SubClassName.ToLower() == "warden")
          this.heroAnimated.transform.localScale = new Vector3(0.92f, 0.92f, 1f);
      }
      this.heroAnimated.transform.localPosition += vector3;
  ...
  public void Show()
  {
    ...
    HeroSelectionManager.Instance.ShowMask(true);
  }
  ...
  public void ShowSingularityCards()
  {
    ...
    this.groupRank.gameObject.SetActive(false);
    this.groupCardback.gameObject.SetActive(false);
    this.groupSingularityCards.gameObject.SetActive(true);
    List<string> stringList = new List<string>();
    for (int index = 0; index < 15; ++index)
    {
      if (index < this.SCD.CardsSingularity.Length && (UnityEngine.Object) this.SCD.CardsSingularity[index] != (UnityEngine.Object) null)
        stringList.Add(this.SCD.CardsSingularity[index].Id);
    }
    stringList.Sort();
    ...
    for (int index = 0; index < 15; ++index)
    {
      if (index < count)
      {
        if ((UnityEngine.Object) this.cardsSING[index] != (UnityEngine.Object) null)
        {
          if (!this.cardsSING[index].gameObject.activeSelf)
            this.cardsSING[index].gameObject.SetActive(true);
          this.cardsSING[index].SetCard(stringList[index], false);
        }
      }
      else if ((UnityEngine.Object) this.cardsSING[index] != (UnityEngine.Object) null && this.cardsSING[index].gameObject.activeSelf)
        this.cardsSING[index].gameObject.SetActive(false);
    }
  }
  ...
  public void DoSuppliesBlock()
  {
    ...
    this.useSuppliesAvailable.text = string.Format(Texts.Instance.GetText("useSuppliesAvailable"), (object) (playerSupplyActual.ToString() + "  "));
