...
public class CardCraftManager : MonoBehaviour
{
  ...
  public void Resize()
  {
    this.exitCraftButton.transform.localPosition = new Vector3((float) (-(double) Globals.Instance.sizeW * 0.5 + 1.0 * (double) Globals.Instance.multiplierX), (float) (-(double) Globals.Instance.sizeH * 0.5 + 3.7200000286102295 * (double) Globals.Instance.multiplierY), this.exitCraftButton.transform.localPosition.z);
  }
  ...
  public void ShowCardCraft(int type = 0)
  {
    ...
    else if (this.craftType == 2)
    {
      ...
      this.filterWindow.gameObject.SetActive(false);
    ...
    else if (this.craftType == 4)
    {
      ...
      {
        bool flag = false;
        for (int key1 = 0; key1 < this.craftCardItemDict.Count; ++key1)
        {
          if (this.craftCardItemDict[key1].cardId == "spyglass" && this.craftCardItemDict[key1].Available)
          {
            GameManager.Instance.ShowTutorialPopup("townItemLoot", this.craftCardItemDict[key1].buttonItem.position, Vector3.zero);
            for (int key2 = 0; key2 < this.craftCardItemDict.Count; ++key2)
            {
              if (key2 != key1)
                this.craftCardItemDict[key2].EnableButton(false);
            }
            flag = true;
            break;
          }
  ...
  public void SetDiscount(int _discount)
  {
    ...
    if (this.discount != 0)
    {
      ...
      if (this.discount > 0)
      {
        ...
        this.discountText.text = string.Format(Texts.Instance.GetText("shopDiscount"), (object) stringBuilder.ToString());
      }
      else
      {
        this.discountText.color = Globals.Instance.ColorColor["redCard"];
        this.discountText.text = string.Format(Texts.Instance.GetText("shopIncrease"), (object) stringBuilder.ToString());
      }
    }
    else
  ...
  {
    this.maxCraftRarity = _maxCraftRarity;
  }
  ...
  private IEnumerator BuyUpgradeCo(string type)
  {
    ...
    if (!cardCraftManager.blocked)
    {
      ...
      AtOManager.Instance.SaveUpgradedCardInAltar(cardCraftManager.heroIndex);
  ...
  private IEnumerator BuyItemCo(string cardId)
  {
    ...
    GameManager.Instance.GenerateParticleTrail(0, Vector3.zero, cardCraftManager.transform.Find("backgroundItems/CharacterItemBlock/CharacterItems/" + name).transform.position);
    yield return (object) Globals.Instance.WaitForSeconds(0.2f);
    ...
    else
    {
      ...
      else
      {
        ...
        else if (cardData.CardClass == Enums.CardClass.Injury || cardData.CardClass == Enums.CardClass.Boon)
        {
          ...
          flag1 = false;
        if (!flag2 && (flag1 || AtOManager.Instance.Sandbox_noMinimumDecksize))
        {
          if (this.CanBuy("Remove"))
    ...
    bool useShopDiscount = true)
  {
    ...
    {
      if (cardName != "")
        rarity = Enum.GetName(typeof (Enums.CardRarity), (object) Globals.Instance.GetCardData(cardName, false).CardRarity);
    ...
    else
    {
      switch (function)
      {
        ...
        case "Craft":
          ...
          num1 = Globals.Instance.GetCraftCost(cardName, discountCraft, discountUpgrade, zoneTier);
          break;
        case "Item":
  ...
  public void ShowItemsForBuy(int pageNum = 1, string itemBought = "")
  {
    ...
    if ((bool) (UnityEngine.Object) TownManager.Instance)
    {
      if (PlayerManager.Instance.PlayerHaveSupply("townUpgrade_6_2") || GameManager.Instance.IsSingularity())
      {
    ...
    if (!this.isPetShop)
    {
      ...
      {
        flag2 = true;
        if (!this.shadyDeal.gameObject.activeSelf)
          this.shadyDeal.gameObject.SetActive(true);
        if ((UnityEngine.Object) this.shadyDealModel != (UnityEngine.Object) null)
          UnityEngine.Object.Destroy((UnityEngine.Object) this.shadyDealModel);
        this.shadyDealModel = UnityEngine.Object.Instantiate<GameObject>(lootData.ShadyModel, new Vector3(0.0f, 0.0f, 0.0f), Quaternion.identity, this.shadyDealModelContainer);
        this.shadyDealModel.GetComponent<BoxCollider2D>().enabled = false;
        this.shadyDealModel.GetComponent<CharacterGOItem>().enabled = false;
        this.shadyDealModel.transform.localScale = new Vector3(lootData.ShadyScaleX, lootData.ShadyScaleY, 1f);
        this.shadyDealModel.transform.localPosition = new Vector3(lootData.ShadyOffsetX, lootData.ShadyOffsetY, 0.0f);
        int childCount = this.shadyDealModel.transform.childCount;
        for (int index = 0; index < childCount; ++index)
        {
          SpriteRenderer component = this.shadyDealModel.transform.GetChild(index).GetComponent<SpriteRenderer>();
          if ((UnityEngine.Object) component != (UnityEngine.Object) null)
          {
            component.sortingOrder = 200 - index;
            component.sortingLayerName = "UI";
          }
        }
      }
      else if (this.shadyDeal.gameObject.activeSelf)
        this.shadyDeal.gameObject.SetActive(false);
    }
    else
    {
  ...
  public bool HavePortraitItemBought(CardCraftItem _CCI)
  {
    ...
    return (UnityEngine.Object) transform != (UnityEngine.Object) null && transform.gameObject.activeSelf;
  }
  ...
  {
    this.rerollChallenge.gameObject.SetActive(state);
  }
  ...
  public void LoadDeck(int slot)
  {
    ...
    foreach (Component component in this.loadDeckCardContainer)
      UnityEngine.Object.Destroy((UnityEngine.Object) component.gameObject);
    this.containerDeckName.text = PlayerManager.Instance.PlayerSavedDeck.DeckTitle[sourceName][slot].ToUpper();
    ...
    for (int index1 = 0; index1 < dictionary.Count; ++index1)
    {
      ...
      CardData cardData = instance.GetCardData(id, false);
  ...
  public int[] GetCardAvailability(string cardId, string shopId)
  {
    ...
    if (cardData1.CardUpgraded != Enums.CardUpgraded.No && cardData1.UpgradedFrom != "")
    {
      ...
      cardData2 = Globals.Instance.GetCardData(cardData1.UpgradedFrom.ToLower(), false);
    }
    else
    {
    ...
    {
      Hero hero = AtOManager.Instance.GetHero(this.heroIndex);
      num1 = !Functions.ListHaveThisBaseCard(cardId, hero.Cards) ? 0 : 1;
      num2 = 1;
    }
  ...
  {
    return this.corruptionArrows[_index].gameObject.activeSelf;
  }
    ...
    int absolutePosition = -1)
  {
    ...
    else
    {
      ...
      else if (this.craftType == 4)
      {
        ...
        if (Functions.TransformIsVisible(this.shadyDealButton.transform))
          this._controllerList.Add(this.shadyDealButton.transform);
        for (int index = 0; index < 4; ++index)
        {
          if ((bool) (UnityEngine.Object) TownManager.Instance)
          {
  ...
  private void GetShadyDealRemaining()
  {
    if (!GameManager.Instance.IsMultiplayer())
    {
      this.shadyRemaining = 4;
    }
    else
    {
      this.shadyRemaining = 0;
      for (int index = 0; index < 4; ++index)
      {
        if (AtOManager.Instance.GetHero(index)?.Owner == NetworkManager.Instance.GetPlayerNick())
          ++this.shadyRemaining;
      }
    }
  }
  ...
  {
    StringBuilder stringBuilder = new StringBuilder();
    if (this.shadyRemaining <= 0)
      stringBuilder.Append("<color=#A86C65>");
    stringBuilder.Append("[");
    stringBuilder.Append(Texts.Instance.GetText("shadyDealLeft"));
    stringBuilder.Append("]");
    this.shadyDealLeft.text = string.Format(stringBuilder.ToString(), (object) this.shadyRemaining);
    if (AtOManager.Instance.GetPlayerDust() < this.shadyDealDust || this.shadyRemaining <= 0)
      this.shadyDealButton.GetComponent<BotonGeneric>().Disable();
    else
      this.shadyDealButton.GetComponent<BotonGeneric>().Enable();
  }
  ...
  private void GetShadyDealDustGold(List<string> itemList)
  {
    ...
    {
      float num2 = (float) this.SetPrice("Item", Enum.GetName(typeof (Enums.CardRarity), (object) Globals.Instance.GetCardData(itemList[index], false).CardRarity), itemList[index], this.craftTierZone, false);
      num1 += num2;
    }
    this.shadyDealGold = Functions.FuncRoundToInt((float) ((double) num1 / (double) itemList.Count / 4.0) * ((float) UnityEngine.Random.Range(12, 16) / 100f));
    this.shadyDealDust = Functions.FuncRoundToInt((float) this.shadyDealGold * ((float) UnityEngine.Random.Range(30, 45) / 10f));
  }
  ...
  private void SetShadyDealDustGoldValues()
  {
    StringBuilder stringBuilder = new StringBuilder();
    stringBuilder.Append("<sprite name=dust> ");
    stringBuilder.Append(this.shadyDealDust);
    this.shadyDealButton.GetComponent<BotonGeneric>().SetText(stringBuilder.ToString());
    stringBuilder.Clear();
    stringBuilder.Append("<sprite name=gold> ");
    stringBuilder.Append(this.shadyDealGold);
    this.shadyDealResult.text = stringBuilder.ToString();
  }
  ...
  public void BuyShadyDeal()
  {
    if (this.blocked)
      return;
    this.SetBlocked(true);
    if (this.shadyRemaining > 0 && this.shadyDealDust < AtOManager.Instance.GetPlayerDust())
    {
      --this.shadyRemaining;
      AtOManager.Instance.PayDust(this.shadyDealDust);
      if (GameManager.Instance.IsMultiplayer())
        AtOManager.Instance.AskForGold(NetworkManager.Instance.GetPlayerNick(), this.shadyDealGold);
      else
        AtOManager.Instance.GivePlayer(0, this.shadyDealGold);
      this.SetShadyDealLeftUses();
    }
    this.SetBlocked(false);
  }
