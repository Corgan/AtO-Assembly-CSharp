...
public class MapManager : MonoBehaviour
{
  ...
  private void DoMapTips()
  {
    ...
    if (str.Trim() != "")
    {
      ...
      this.tipText.text = Regex.Replace(stringBuilder.ToString(), "\\<icon=(\\w+)\\>", " <size=+1.2><voffset=-.5><sprite name=$1><voffset=0></size>");
    }
  ...
  private void DrawMap()
  {
    ...
    this.DrawNodes();
    if (!this.SetPositionInCurrentNode())
      return;
    this.ShowTMPRoads();
    if (!GameManager.Instance.IsMultiplayer() || NetworkManager.Instance.IsMaster())
    {
  ...
  {
    this.StartCoroutine(this.GetMapNodesCo(useDelay));
  }
  ...
  private IEnumerator GetMapNodesCo(bool useDelay)
  {
    ...
    if (!(nodeId == ""))
    {
      ...
      foreach (Transform transform1 in this.worldTransform)
      {
        ...
        if (GameManager.Instance.IsObeliskChallenge() || !(lower1 == "challengecommonmap") && !(lower1 == "challengehighmap") && !(lower1 == "challengefinalmap") && !(lower1.Substring(0, 2) == "ol") && !(lower1.Substring(0, 2) == "oh") && !(lower1.Substring(0, 2) == "of"))
        {
          foreach (Transform transform2 in transform1)
          {
            if (transform2.gameObject.name == "Nodes")
            {
              transform2.transform.localPosition = new Vector3(transform2.transform.localPosition.x, transform2.transform.localPosition.y, -2f);
              foreach (Transform transform3 in transform2)
              {
  ...
  private void DrawNodes()
  {
    ...
    if (Globals.Instance.ShowDebug)
      Functions.DebugLogGD("+++ Draw Nodes +++ (" + this.mapNodes.Count.ToString() + ")", "map");
    for (int index = 0; index < this.mapNodes.Count; ++index)
    {
      Node node = this.mapNode[this.mapNodes[index]];
      Functions.DebugLogGD("->" + node.nodeData.NodeId + " " + AtOManager.Instance.gameNodeAssigned.ContainsKey(node.nodeData.NodeId).ToString(), "map");
    ...
    if (GameManager.Instance.IsObeliskChallenge())
    {
      foreach (Transform transform1 in this.worldTransform)
      {
        if (transform1.gameObject.activeSelf)
        {
          ...
          if (num1 > 0 && num1 == num2)
          {
            Debug.LogError((object) "No nodes to show, reassign nodes");
  ...
  private void AssignGameNodes()
  {
    ...
    for (int index = 0; index < this.mapNode.Count; ++index)
    {
      ...
      if ((UnityEngine.Object) _node == (UnityEngine.Object) null)
        Debug.LogError((object) ("Node not found at element " + index.ToString()));
      else if (AtOManager.Instance.gameNodeAssigned.ContainsKey(_node.nodeData.NodeId) && AtOManager.Instance.mapVisitedNodes.Contains(_node.nodeData.NodeId))
      {
  ...
  private bool SetPositionInCurrentNode()
  {
    ...
    else if (str == "spider_8" && AtOManager.Instance.PlayerHasRequirement(Globals.Instance.GetRequirementData("spidercaveout")) && !this.NodeExists(Globals.Instance.GetNodeData("spider_9")))
    {
      if (Globals.Instance.ShowDebug)
        Functions.DebugLogGD("Exit", "map");
      this.StartCoroutine(this.ChangeZoneBecauseStuck(str));
    }
    else if (str == "secta_5" && AtOManager.Instance.PlayerHasRequirement(Globals.Instance.GetRequirementData("belphyordead")) && !this.NodeExists(Globals.Instance.GetNodeData("secta_8")))
    {
      if (Globals.Instance.ShowDebug)
        Functions.DebugLogGD("Exit", "map");
      this.StartCoroutine(this.ChangeZoneBecauseStuck(str));
    }
    else if (str == "sen_34" || str == "velka_33" || str == "aqua_36" || str == "ulmin_40" || str == "faen_39")
    {
      if (Globals.Instance.ShowDebug)
        Functions.DebugLogGD("Exit", "map");
      this.StartCoroutine(this.ChangeZoneBecauseStuck(str));
    }
    else if (str == "voidhigh_13" && AtOManager.Instance.bossesKilledName != null && AtOManager.Instance.bossesKilledName.Any<string>((Func<string, bool>) (s => s.StartsWith("lordhanshek", StringComparison.OrdinalIgnoreCase))))
    {
      if (!AtOManager.Instance.bossesKilledName.Any<string>((Func<string, bool>) (s => s.StartsWith("archonnihr", StringComparison.OrdinalIgnoreCase))))
      {
        AtOManager.Instance.SetCombatData(Globals.Instance.GetCombatData("evoidhigh_13b"));
        this.DoCombat(AtOManager.Instance.GetCurrentCombatData());
        return false;
      }
  ...
  private IEnumerator ChangeZoneBecauseStuck(string nodeName)
  {
    ...
    if (Globals.Instance.ShowDebug)
      Debug.Log((object) nameof (nodeName));
    switch (nodeName)
    {
      case "spider_8":
      ...
      default:
        ...
        this.TravelToThisNode(this.GetNodeFromId(nodeName));
        break;
    }
  }
    ...
    bool challenge = false)
  {
    ...
    if (this.roads.ContainsKey(key))
    {
      ...
      if (challenge)
      {
        ...
        component.textureScale = this.vectorLineTexture;
  ...
  {
    return !((UnityEngine.Object) _nodeData == (UnityEngine.Object) null) && AtOManager.Instance.gameNodeAssigned.ContainsKey(_nodeData.NodeId);
  }

  public bool NodeVisible(NodeData _nodeData)
  {
    return !((UnityEngine.Object) _nodeData.NodeRequirement != (UnityEngine.Object) null) || AtOManager.Instance.PlayerHasRequirement(_nodeData.NodeRequirement);
  }
  ...
  {
    this.mapNode[_nodeDestinationId].HighlightNode(_status);
  }

  public void HighlightNodeMP(Node _nodeDestination, bool _status)
  {
    this.photonView.RPC("NET_HightlightNode", RpcTarget.Others, (object) _nodeDestination.nodeData.NodeId, (object) _status);
  }

  [PunRPC]
  private void NET_HightlightNode(string _nodeDestination, bool _status)
  {
    this.mapNode[_nodeDestination].HighlightNode(_status);
  }
  ...
  {
    this.eventManager.NET_OptionSelected(_playerNick, _option);
  }

  [PunRPC]
  public void NET_Event_SelectAnswer(int _answerId)
  {
    this.eventManager.NET_SelectAnswer(_answerId);
  }
  ...
  {
    this.GenerateRandomStringBatch((int) _total, _seed);
  }

  public void SetRandomIndexRandom()
  {
    this.randomIndex = UnityEngine.Random.Range(0, this.randomStringArr.Count);
  }
  ...
  {
    return Functions.Random(min, max, this.GetRandomString());
  }
  ...
  {
    this.StartCoroutine(this.NET_ShareConflictOrderCo(_playerChoosing));
  }
  ...
  {
    this.conflict.SelectOptionFromOutside(_option);
  }
  ...
  {
    this.StartCoroutine(this.ResultConflictCo(_playerWin));
  }
  ...
  {
    return !((UnityEngine.Object) this.unlockGO == (UnityEngine.Object) null) && this.unlockGO.gameObject.activeSelf;
  }
  ...
  {
    return this.maskObject.gameObject.activeSelf || this.maskMP.gameObject.activeSelf;
  }
