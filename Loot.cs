...
public static class Loot
{
  public static List<string> GetLootItems(string _itemListId, string _idAux = "")
  {
    ...
    for (int index2 = 0; index2 < num1 && ts1.Count < lootData.NumItems; ++index2)
    {
      if (index2 < lootData.LootItemTable.Length)
      {
        ...
        if ((double) Random.Range(0, 100) < (double) lootItem.LootPercent)
        {
          ...
          {
            string str = Loot.StablishLootMisc(lootItem);
            if (str != "")
              ts1.Add(str);
            else if ((Object) lootItem.LootCard != (Object) null)
    ...
    {
      bool flag = true;
      for (int index4 = 0; index4 < ts1.Count; ++index4)
      {
        if (ts1[index4] == "spyglass")
        {
          flag = false;
          break;
        }
  ...
  {
    string lower = lootItem.LootMisc.ToLower();
    string str = "";
    if (lower == "scrappyrobot")
      str = ScrappyRobotResolver.GetScrappyRobot(AtOManager.Instance.GetPlayerRequeriments());
    return str;
  }
}
