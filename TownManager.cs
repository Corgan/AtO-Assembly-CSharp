...
public class TownManager : MonoBehaviour
{
  ...
  private void Start()
  {
    ...
    {
      this.buildingCart.DisableThis();
      this.townUpgrades.gameObject.SetActive(false);
    }
  ...
  private void CreateTutorialBanner()
  {
    ...
    stringBuilder2.Append(string.Format(Texts.Instance.GetText("townTutorialStep0"), (object) Globals.Instance.GetCardData("fireball", false).CardName, (object) "Evelyn"));
    if (flag1)
    ...
    stringBuilder2.Append(string.Format(Texts.Instance.GetText("townTutorialStep1"), (object) Globals.Instance.GetCardData("faststrike", false).CardName, (object) "Magnus"));
    if (flag2)
      stringBuilder2.Append("</size></color>");
    bool flag3 = false;
    stringBuilder2.Append(stringBuilder1.ToString());
    ...
    stringBuilder2.Append(string.Format(Texts.Instance.GetText("townTutorialStep2"), (object) Globals.Instance.GetCardData("spyglass", false).CardName, (object) "Andrin"));
    if (flag3)
      stringBuilder2.Append("</size></color>");
  ...
  public void ShowButtons(bool state)
  {
    ...
    if (flag || AtOManager.Instance.GetNgPlus() > 2 || GameManager.Instance.IsSingularity())
    {
    ...
    if (PlayerManager.Instance.PlayerRuns != null)
    {
      for (int i = PlayerManager.Instance.PlayerRuns.Count - 1; i >= 0; --i)
      {
        if (i >= 0)
        {
          ...
          if (_playerRun.MadnessLevel <= 2 && !_playerRun.ObeliskChallenge && !_playerRun.Singularity && !_playerRun.SandboxEnabled && (PlayerManager.Instance.TreasuresClaimed == null || !PlayerManager.Instance.IsTreasureClaimed(_playerRun.Id)))
          {
            if (index <= 2)
            {
  ...
  {
    this.townUpgradeWindow.ModifySupplyQuantity(quantity);
  }
    ...
    bool shoulderRight = false)
  {
    ...
    else
    {
      ...
      if (this.buildingCart.transform.gameObject.activeSelf)
      ...
      if (this.townUpgrades.gameObject.activeSelf)
