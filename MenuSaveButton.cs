...
public class MenuSaveButton : MonoBehaviour
{
  ...
  public void SetGameData(GameData _gameData)
  {
    ...
    if (Functions.CheckIfSavegameIsCompatible(this.gameData) != "")
    {
    ...
    stringBuilder.Clear();
    NodeData nodeData = Globals.Instance.GetNodeData(this.gameData.CurrentMapNode);
    ...
    CultureInfo infoByTwoLetterCode = Functions.GetCultureInfoByTwoLetterCode(Globals.Instance.CurrentLang);
    string[] strArray = (infoByTwoLetterCode == null ? _gameData.GameDate : DateTime.Parse(_gameData.GameDate).ToString("g", (IFormatProvider) infoByTwoLetterCode)).Split(' ', StringSplitOptions.None);
  ...
  public void SelectThis()
  {
    ...
    if (!this.active)
      SceneStatic.LoadByName(this.slotNum % 12 < 6 ? "HeroSelection" : "Lobby");
    else
  ...
  private void DeleteAction()
  {
    ...
    if (!AlertManager.Instance.GetConfirmAnswer())
      return;
    SaveManager.DeleteGame(this.slotNum, true);
    this.ShowNGPlus(false);
