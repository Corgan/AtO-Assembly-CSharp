...
public class LobbyManager : MonoBehaviourPunCallbacks
{
  ...
  public void InitLobby()
  {
    ...
    if ((UnityEngine.Object) this.RoomT != (UnityEngine.Object) null && this.RoomT.gameObject.activeSelf)
      this.RoomT.gameObject.SetActive(false);
    if ((UnityEngine.Object) this.CreateRoomT != (UnityEngine.Object) null && this.CreateRoomT.gameObject.activeSelf)
      this.CreateRoomT.gameObject.SetActive(false);
    if ((UnityEngine.Object) this.JoinRoomT != (UnityEngine.Object) null && this.JoinRoomT.gameObject.activeSelf)
      this.JoinRoomT.gameObject.SetActive(false);
    if ((UnityEngine.Object) this.regionsDisconnect != (UnityEngine.Object) null && this.regionsDisconnect.gameObject.activeSelf)
    ...
    else
    {
      ...
      {
        this.UICrossPlay.isOn = false;
        this.UICrossPlay.interactable = false;
        this.lockCrossPlay.gameObject.SetActive(true);
        this.lockCrossBackground.gameObject.SetActive(false);
      }
      else
      {
        this.UICrossPlay.interactable = true;
        this.UICrossPlay.isOn = GameManager.Instance.ConfigCrossPlayEnabled;
        this.lockCrossPlay.gameObject.SetActive(false);
        this.lockCrossBackground.gameObject.SetActive(true);
      }
  ...
  {
    if ((UnityEngine.Object) this.regions != (UnityEngine.Object) null && this.regions.gameObject.activeSelf)
      this.regions.gameObject.SetActive(false);
  ...
  private void ShowConnectedStatus()
  {
    ...
    stringBuilder.Append("]");
    this.SetStatus(true, stringBuilder.ToString());
  }
  ...
  public void GetRoomId()
  {
    ...
    if (!(upper.Trim() != ""))
      return;
    for (int index = 0; index < this.RoomListButtons.Count; ++index)
    {
      if (upper == this.RoomListButtons[index].RoomName)
      {
        NetworkManager.Instance.JoinRoom(upper);
        return;
      }
    }
    this.StartCoroutine(this.RoomByIdErrorAlert());
  }
  ...
  public void ShowRoom()
  {
    ...
    for (int index = 0; index < this.roomSlots.Length; ++index)
    {
      ...
      this.roomSlotsIcon[index].gameObject.SetActive(false);
    }
  ...
  private void DrawLobbyNames()
  {
    ...
    foreach (Player player in NetworkManager.Instance.PlayerList)
    {
      ...
      if ((UnityEngine.Object) this.roomSlots[slot] != (UnityEngine.Object) null)
      {
        this.roomSlots[slot].gameObject.SetActive(true);
        this.roomSlots[slot].text = stringBuilder.ToString();
      }
      if ((UnityEngine.Object) this.roomSlotsImage[slot] != (UnityEngine.Object) null)
      {
        this.roomSlotsImage[slot].gameObject.SetActive(true);
        this.roomSlotsImage[slot].color = Functions.HexToColor(NetworkManager.Instance.GetColorFromNick(player.NickName));
      }
      if ((UnityEngine.Object) this.roomSlotsIcon[slot] != (UnityEngine.Object) null)
      {
        this.roomSlotsIcon[slot].gameObject.SetActive(true);
        this.roomSlotsIcon[slot].sprite = NetworkManager.Instance.GetSlotPlatformImage(slot);
      }
      if (slot > 0)
      {
        if (PhotonNetwork.IsMasterClient)
          this.roomSlotsKick[slot].gameObject.SetActive(true);
        else
          this.roomSlotsKick[slot].gameObject.SetActive(false);
      }
      if (Globals.Instance.ConfigPlatform.ToString() != NetworkManager.Instance.GetPlatformString(slot))
        flag1 = false;
      ++slot;
    }
    int num = slot;
    bool flag2 = true;
    if (slot < (int) PhotonNetwork.CurrentRoom.MaxPlayers)
    {
      flag2 = false;
      for (int index = slot; index < (int) PhotonNetwork.CurrentRoom.MaxPlayers; ++index)
      {
        stringBuilder.Clear();
        stringBuilder.Append("<color=#999>");
        stringBuilder.Append(Texts.Instance.GetText("openSlot"));
        stringBuilder.Append("</color>");
        this.roomSlots[index].gameObject.SetActive(true);
        this.roomSlots[index].text = stringBuilder.ToString();
        this.roomSlotsImage[index].gameObject.SetActive(false);
        this.roomSlotsIcon[index].gameObject.SetActive(false);
        this.roomSlotsKick[index].gameObject.SetActive(false);
        ++slot;
      }
    }
    for (int index = slot; index < this.roomSlots.Length; ++index)
    {
      this.roomSlots[index].gameObject.SetActive(false);
      this.roomSlotsImage[index].gameObject.SetActive(false);
      this.roomSlotsIcon[index].gameObject.SetActive(false);
    }
    this.roomWaiting.text = flag2 ? Texts.Instance.GetText("roomFull") : Texts.Instance.GetText("waitingPlayers");
    if (PhotonNetwork.IsMasterClient)
    {
      if (num > 1)
    ...
    {
      if (flag1 || Startup.isLoggedIn && GameManager.Instance.ConfigCrossPlayEnabled)
        return;
  ...
  {
    object customProperty = room.CustomProperties[(object) "crossplay"];
    if (customProperty != null && customProperty.ToString() == "1")
      return GameManager.Instance.ConfigCrossPlayEnabled;
    string str = "PC";
    if (room.CustomProperties[(object) "platform"] != null)
      str = room.CustomProperties[(object) "platform"].ToString();
    return Globals.Instance.ConfigPlatform.ToString() == str;
  }

  public void RoomReceived(RoomInfo room)
  {
    if (!this.IsRoomAvailableForMultiPlatform(room))
      return;
    ...
    if (room.IsOpen && room.PlayerCount > 0 && room.PlayerCount < (int) room.MaxPlayers)
    {
      ...
      if (room.CustomProperties.ContainsKey((object) "version"))
        roomListButton.SetRoomVersion(room.CustomProperties[(object) "version"].ToString(), room.CustomProperties[(object) "platform"]?.ToString());
      if (room.CustomProperties.ContainsKey((object) "crossplay"))
        roomListButton.SetRoomCrossplayPlatform(room.CustomProperties[(object) "crossplay"].ToString(), room.CustomProperties[(object) "platform"].ToString());
  ...
  public void ControllerMovement(bool goingUp = false, bool goingRight = false, bool goingDown = false, bool goingLeft = false)
  {
    ...
    if (Functions.TransformIsVisible(this.regions))
    {
      ...
      this._controllerList.Add(this.regions.GetChild(2));
      this._controllerList.Add(this.regions.GetChild(3));
    }
