...
public class FinishRunManager : MonoBehaviour
{
  ...
  public void CalculateFinishRunReward()
  {
    ...
    this.gameScore = num13 + Functions.FuncRoundToInt((float) (num13 * madnessScoreMultiplier / 100));
    this.finalScore.text = this.gameScore.ToString();
    this.StartCoroutine(this.RemoveDoubleDots());
    bool flag = true;
    ...
    if (flag)
    {
      ...
      {
        if (this.gameScore > PlayerManager.Instance.BestScore)
        {
    ...
    this.retentionText.text = num17.ToString() + (Functions.SpaceBeforePercentSign() ? " %" : "%");
    if (num3 == 0)
    {
      int num18;
      int num19 = num18 = 0;
    ...
    if (this.dustSummary < 0)
      this.dustSummary = 0;
    if (GameManager.Instance.IsSingularity() || num14 > 2 || GameManager.Instance.IsObeliskChallenge() || SandboxManager.Instance.IsEnabled())
    {
    ...
    _playerRun.GameUniqueId = AtOManager.Instance.GetGameUniqueId();
    _playerRun.GameSeed = AtOManager.Instance.GetGameId();
    _playerRun.gameDate = DateTime.Now.ToString();
    _playerRun.Singularity = GameManager.Instance.IsSingularity();
    ...
    _playerRun.FinalScore = this.gameScore;
    ...
    _playerRun.SingularityMadness = AtOManager.Instance.GetSingularityMadness();
  ...
  private IEnumerator RemoveDoubleDots()
  {
    ...
    for (int index = 0; index < this.titlesTexts.Count; ++index)
      this.titlesTexts[index].text.Replace(':', ' ').Replace('：', ' ');
  }
