...
public class TomeRunDetails : MonoBehaviour
{
  ...
  private void SetDescription()
  {
    ...
    if (!this.playerRun.ObeliskChallenge)
    {
      int num = this.playerRun.Singularity ? this.playerRun.SingularityMadness : MadnessManager.Instance.CalculateMadnessTotal(this.playerRun.MadnessLevel, this.playerRun.MadnessCorruptors);
      if (num > 0)
      {
        this.madness.gameObject.SetActive(true);
        this.madnessText.text = num.ToString();
      }
    }
    ...
    stringBuilder.Clear();
    int num1 = this.playerRun.CommonCorruptions + this.playerRun.UncommonCorruptions + this.playerRun.RareCorruptions + this.playerRun.EpicCorruptions;
