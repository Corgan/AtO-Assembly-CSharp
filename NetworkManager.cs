...
public class NetworkManager : MonoBehaviourPunCallbacks
{
  ...
  private void Awake()
  {
    ...
    this.PlayerPlatformDict = new Dictionary<string, string>();
    this.PlayerPDXUserDict = new Dictionary<string, string>();
    this.PlayerSkuList = new Dictionary<string, List<string>>();
    this.PlayerPing = new Dictionary<string, string>();
  ...
  public bool IsOnRoom() => this.onRoom;

  public string GetPing() => PhotonNetwork.IsConnected ? PhotonNetwork.GetPing().ToString() : "";

  public void SendPing(string ping)
  {
    this.photonView.RPC("SendPingCo", RpcTarget.All, (object) this.GetPlayerNick(), (object) ping, (object) (byte) this.GetMyPosition());
  }

  [PunRPC]
  public void SendPingCo(string _nick, string _ping, byte _slot)
  {
    if (this.PlayerPing.ContainsKey(_nick))
      this.PlayerPing[_nick] = _ping;
    else
      this.PlayerPing.Add(_nick, _ping);
    AlertManager.Instance.UpdatePlayer((int) _slot);
  }
  ...
  {
    return this.PlayerSkuList != null && this.PlayerSkuList.ContainsKey(_nick) && this.PlayerSkuList[_nick] != null && this.PlayerSkuList[_nick].Contains(_sku);
  }
  ...
  public void Connect(string region = "")
  {
    ...
    else
    {
      ...
      this.PlayerPlatformDict.Clear();
      this.PlayerSkuList.Clear();
      this.PlayerPing.Clear();
      this.realName = !(SteamManager.Instance.steamName != "") ? PlayerManager.Instance.GetPlayerName() : SteamManager.Instance.steamName.Replace(" ", "").Replace("_", "");
      PhotonNetwork.NickName = this.realName;
      for (int index = 0; this.PlayerNickRealDict.ContainsKey(PhotonNetwork.NickName) && index < 20; ++index)
        PhotonNetwork.NickName = this.realName + "_" + Functions.RandomString(4f);
      ...
      if (!this.PlayerPlatformDict.ContainsKey(PhotonNetwork.NickName))
        this.PlayerPlatformDict.Add(PhotonNetwork.NickName, Globals.Instance.ConfigPlatform.ToString());
  ...
  {
    this.photonView.RPC("GotChat", RpcTarget.Others, (object) Encoding.UTF8.GetBytes(_text), (object) showAlertIfClosed, (object) _playerNick);
  }

  [PunRPC]
  public void GotChat(byte[] _text, bool showAlertIfClosed, string _playerNick)
  {
    if (_playerNick.IsNullOrEmpty() || !NetworkManager.Instance.IsPlayerMutedByNick(_playerNick))
      ChatManager.Instance.ChatText(Encoding.UTF8.GetString(_text), showAlertIfClosed);
    else
  ...
  public void CreateRoom(string roomName, string roomPlayers, string roomPassword = "", string lfm = "")
  {
    ...
    else if (GameManager.Instance.GameType == Enums.GameType.WeeklyChallenge)
    {
      ...
      stringBuilder.Append(" <color=#666>|</color> ");
      str = !(AtOManager.Instance.GetGameId() != "") ? AtOManager.Instance.GetWeeklyName(Functions.GetCurrentWeeklyWeek()) : AtOManager.Instance.GetWeeklyName(AtOManager.Instance.GetWeekly());
    }
    ...
    hashtable.Add((object) "platform", (object) Globals.Instance.ConfigPlatform.ToString());
    hashtable.Add((object) "crossplay", GameManager.Instance.ConfigCrossPlayEnabled ? (object) "1" : (object) "0");
  ...
  {
    return PhotonNetwork.NetworkingClient.CurrentRoom.CustomProperties[(object) "description"].ToString();
  }
  ...
  {
    return PhotonNetwork.NetworkingClient.CurrentRoom != null ? PhotonNetwork.NetworkingClient.CurrentRoom.Name : "";
  }
  ...
  public bool VersionMatch()
  {
    for (int index = 0; index < this.PlayerPositionList.Length; ++index)
    {
      if (this.PlayerPositionList[index] != null && (this.PlayerVersionDict == null || !this.PlayerVersionDict.ContainsKey(this.PlayerPositionList[index]) || !Functions.CompatibleVersion(this.PlayerVersionDict[this.PlayerPositionList[index]], GameManager.Instance.gameVersion)))
        return false;
    }
    return true;
  }
  ...
  {
    return slot > -1 && this.PlayerPositionList != null && slot < this.PlayerPositionList.Length && !this.PlayerPositionList[slot].IsNullOrEmpty() ? this.PlayerPlatformDict[this.PlayerPositionList[slot]] : "";
  }

  public Sprite GetSlotPlatformImage(int slot)
  {
    bool flag = false;
    if (this.PlayerPlatformDict == null || !this.PlayerPlatformDict.ContainsKey(this.PlayerPositionList[slot]))
      flag = true;
    else if (this.PlayerPlatformDict[this.PlayerPositionList[slot]] == Globals.Instance.ConfigPlatform.ToString())
      flag = true;
    return flag ? this.GetPlatformImage(Globals.Instance.ConfigPlatform.ToString()) : this.GetPlatformImage();
  }
  ...
  private Sprite GetPlatformImage(string platform = "")
  {
    ...
    return platform == Enums.Platform.XBOX.ToString() ? Globals.Instance.xboxLogo : Globals.Instance.pdxLogo;
  }
  ...
  {
    return nick != "" && nick != null && this.PlayerNickRealDict != null && this.PlayerNickRealDict.ContainsKey(nick) ? this.PlayerNickRealDict[nick] : nick;
  }

  public string GetPlayerVersion(string nick)
  {
    return nick != "" && nick != null && this.PlayerVersionDict != null && this.PlayerVersionDict.ContainsKey(nick) ? this.PlayerVersionDict[nick] : nick;
  }

  public string GetPlayerNick()
  {
    return PhotonNetwork.IsConnected ? this.GetPlayer().NickName : Globals.Instance.DefaultNickName;
  }
  ...
  {
    return this.PlayerPositionList != null && position > -1 && position < this.PlayerPositionList.Length ? this.PlayerPositionList[position] : "";
  }
  ...
  {
    return this.ColorFromPosition(this.GetPlayerListPosition(nick));
  }
  ...
  {
    this.photonView.RPC("NET_NetworkContinue", RpcTarget.Others, (object) key, (object) auxValue);
  }
  ...
  {
    this.photonView.RPC("NET_SetPlayerManualReady", RpcTarget.MasterClient, (object) status, (object) this.GetPlayerNick());
  }
  ...
  public void SetPlayerDivinationReady(string nickname)
  {
    ...
    if (flag)
    {
      AtOManager.Instance.LaunchRewards(true);
    }
    else
    {
  ...
  public override void OnJoinedRoom()
  {
    ...
    this.photonView.RPC("SetPlayerPosition", RpcTarget.MasterClient, (object) this.realName, (object) this.GetPlayerNick(), (object) GameManager.Instance.gameVersion, (object) Globals.Instance.ConfigPlatform, (object) Startup.userId);
  }
  ...
  {
    this.PlayerPositionList = new string[numPlayers];
  }

  [PunRPC]
  public void SetPlayerPosition(
    string playerNickReal,
    string playerNick,
    string gameVersion = "0.0.0",
    Enums.Platform platform = Enums.Platform.PC,
    string pdxUserId = "")
  {
    if (GameManager.Instance.GetDeveloperMode())
      Debug.Log((object) ("[SetPlayerPosition] playerNickReal -> " + playerNickReal + " // playerNick -> " + playerNick + " // gameVersion -> " + gameVersion + " // platform -> " + platform.ToString() + " // pdxUserId -> " + pdxUserId));
    bool flag = false;
    for (int index = 0; index < this.PlayerPositionList.Length; ++index)
    {
    ...
    if (!this.PlayerPlatformDict.ContainsKey(playerNick))
      this.PlayerPlatformDict.Add(playerNick, platform.ToString());
    if (!this.PlayerPDXUserDict.ContainsKey(playerNick))
      this.PlayerPDXUserDict.Add(playerNick, pdxUserId);
    ...
    string[] array4 = new string[this.PlayerPlatformDict.Count];
    this.PlayerPlatformDict.Values.CopyTo(array4, 0);
    string json4 = JsonHelper.ToJson<string>(array4);
    string[] array5 = new string[this.PlayerPDXUserDict.Count];
    this.PlayerPDXUserDict.Values.CopyTo(array5, 0);
    string json5 = JsonHelper.ToJson<string>(array5);
    this.photonView.RPC("NET_SharePlayerNickReal", RpcTarget.Others, (object) json1, (object) json2, (object) json3, (object) json4, (object) json5);
    this.photonView.RPC("NET_SharePlayerPositionList", RpcTarget.Others, (object) JsonHelper.ToJson<string>(this.PlayerPositionList));
  ...
  {
    return this.PlayerPositionList != null && this.PlayerPositionList[0] != null && this.PlayerPositionList[0] == playerNick;
  }
  ...
  [PunRPC]
  public void NET_SharePlayerNickReal(
    string _keys,
    string _values,
    string _versions,
    string _platforms,
    string _pdxs)
  {
    ...
    string[] strArray4 = JsonHelper.FromJson<string>(_platforms);
    string[] strArray5 = JsonHelper.FromJson<string>(_pdxs);
    this.PlayerNickRealDict = new Dictionary<string, string>();
    this.PlayerVersionDict = new Dictionary<string, string>();
    this.PlayerPlatformDict = new Dictionary<string, string>();
    this.PlayerPDXUserDict = new Dictionary<string, string>();
    for (int index = 0; index < strArray1.Length; ++index)
    {
      ...
      this.PlayerPlatformDict.Add(strArray1[index], strArray4[index]);
      this.PlayerPDXUserDict.Add(strArray1[index], strArray5[index]);
    }
  }
  ...
  {
    StringBuilder stringBuilder = new StringBuilder();
    stringBuilder.Append(NetworkManager.Instance.GetPlatformString(_playerSlot));
    stringBuilder.Append('_');
    stringBuilder.Append(NetworkManager.Instance.GetPlayerNickReal(this.PlayerList[_playerSlot].NickName));
    return stringBuilder.ToString();
  }

  public void DoMute(int _playerSlot)
  {
    string platformNick = this.GetPlatformNick(_playerSlot);
    if (!this.PlayerMuteList.Contains(platformNick))
      this.PlayerMuteList.Add(platformNick);
  ...
  public void DoUnmute(int _playerSlot)
  {
    string platformNick = this.GetPlatformNick(_playerSlot);
    if (this.PlayerMuteList.Contains(platformNick))
      this.PlayerMuteList.Remove(platformNick);
    SaveManager.SaveMutedPlayers();
  }

  public bool IsPlayerMutedByNick(string _nick)
  {
    return this.PlayerMuteList.Contains(this.GetPlatformNick(this.GetPlayerListPosition(_nick)));
  }

  public bool IsPlayerMutedBySlot(int _playerSlot)
  {
    return this.PlayerMuteList.Contains(this.GetPlatformNick(_playerSlot));
  }
}
